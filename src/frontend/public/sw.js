...
self.addEventListener('fetch',event=>{
  ...
  if(url.hostname.endsWith('tile.openstreetmap.org')){
    event.respondWith(caches.open(MAP_CACHE).then(cache=>cache.match(req).then(hit=>hit||fetch(req).then(res=>{cache.put(req,res.clone());return res}))));return;
  }
